...
const int arrayLength3 = 7; //Length for temp type and value
const int arrayLength4 = 7; //Length for temp value only
...
char temperature[arrayLength3]; //Char array storing temp type and value
char tempType; //Shows temperature type in Fahrenheit or Celsius
double tempValue = 0.0; //Initialize temperature value
double averageTemp = 0.0; //Initialize average temperature
	...
	for(int index = 0; index < numSamples; index++)
	{
		...
		fin >> temperature; //Read in the temperature info
        	...
        	//If temperature is not F or C, give error message
            	...
            	{
            		cout << "Error: temperature not in F or C.";
        	...
        	if(temperature[0] == 'F')
            	{
            		char tempOnly[arrayLength3-1];//Temp value only

            		//Get rid of the temp type letter
            		for(int i = 1; i < arrayLength3; i++)
                		tempOnly[i-1] = temperature[i];

            		tempValue = atof(tempOnly); //Convert to double
...
double convertTemp(double tempValue)
{
	double tempInC = 0.0;//Calculate temp in Celcius
	tempInC = (tempValue-32)*(5.0/9.0); //Formula
	return (tempInC);//Return temp in Celcius
}
